...
int regex_match(char *regular_expression, char *line_text) {
	...
	else if ( ( current_char != 0x5C && next_char == 0x28 ) || current_char == 0x28 ) { 
...
char *create_group(char *regex_ptr) {
	...
	*/
	...
	if ( *read_ptr == 0x00 ) {
		fprintf(stderr, "Invalid regular expression given: no closing parentheses in grouping [%s]", regular_expression);
		exit(0);
	}
